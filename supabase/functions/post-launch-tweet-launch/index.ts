...
const TWEET_EVENT = 'launch_tweet_posted_trylaunchai';
...
function xCreds() {
  // Credentials for the @trylaunchai X account (separate app from the
  // Vibe Coded It account used by post-launch-tweet).
  const consumerKey = Deno.env.get('LAUNCH_TWITTER_CONSUMER_KEY');
  const consumerSecret = Deno.env.get('LAUNCH_TWITTER_CONSUMER_SECRET');
  const accessToken = Deno.env.get('LAUNCH_TWITTER_ACCESS_TOKEN');
  const accessTokenSecret = Deno.env.get('LAUNCH_TWITTER_ACCESS_TOKEN_SECRET');
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error('post-launch-tweet-launch error:', error);
