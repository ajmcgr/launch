...
DROP TRIGGER IF EXISTS trg_collection_items_touch ON public.collection_items;
CREATE TRIGGER trg_collection_items_touch
AFTER INSERT OR DELETE OR UPDATE ON public.collection_items
FOR EACH ROW EXECUTE FUNCTION public.touch_collection_updated_at();

CREATE OR REPLACE FUNCTION public.touch_collection_self()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN NEW.updated_at = now(); RETURN NEW; END;
$$;

DROP TRIGGER IF EXISTS trg_collections_touch_self ON public.collections;
CREATE TRIGGER trg_collections_touch_self
BEFORE UPDATE ON public.collections
FOR EACH ROW EXECUTE FUNCTION public.touch_collection_self();

CREATE OR REPLACE FUNCTION public.collection_is_visible(_collection_id uuid)
...
CREATE OR REPLACE FUNCTION public.collection_is_owned(_collection_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.collections c
...
DROP POLICY IF EXISTS "Collections visible to owner or if public" ON public.collections;
...
DROP POLICY IF EXISTS "Items visible when parent visible" ON public.collection_items;
CREATE POLICY "Items visible when parent visible"
  ON public.collection_items FOR SELECT
  USING (public.collection_is_visible(collection_id));

DROP POLICY IF EXISTS "Owner inserts items" ON public.collection_items;
CREATE POLICY "Owner inserts items"
  ON public.collection_items FOR INSERT
  WITH CHECK (public.collection_is_owned(collection_id));

DROP POLICY IF EXISTS "Owner updates items" ON public.collection_items;
CREATE POLICY "Owner updates items"
  ON public.collection_items FOR UPDATE
  USING (public.collection_is_owned(collection_id));

DROP POLICY IF EXISTS "Owner deletes items" ON public.collection_items;
CREATE POLICY "Owner deletes items"
  ON public.collection_items FOR DELETE
  USING (public.collection_is_owned(collection_id));

-- Optional seed: a public "Editor's Picks" collection (commented out by default)
-- INSERT INTO public.collections (user_id, name, description, is_public, slug)
